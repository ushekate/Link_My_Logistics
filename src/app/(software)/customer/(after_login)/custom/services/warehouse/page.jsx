...
import { services_3pl_cfs, services_3pl_warehouse } from "@/constants/services";

export default function Services3PLPage() {
	const router = useRouter();
	const { setTitle } = useSidebar();
	const [searchQuery, setSearchQuery] = useState('');
	const [filteredServices, setFilteredServices] = useState(services_3pl_warehouse);

	useEffect(() => {
		setTitle('Warehouse Services')
	}, []);

	const handleSearch = (e) => {
		const value = e.target.value;
		if (value === '') {
			setFilteredServices(services_3pl_cfs);
		} else {
			setFilteredServices(services_3pl_cfs.filter((service) => service.title.toLowerCase().includes(value.toLowerCase())));
		}
		...
		<section className="p-4">
			...
			<div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 py-10">
				{filteredServices.map((service, index) => (
					<div key={index} className="border-2 border-[var(--primary)] p-4 rounded-xl flex flex-col gap-2">
						<service.icon className="bg-[var(--background-2)] p-2 w-10 h-10 text-[var(--light-primary)] rounded-md" />
