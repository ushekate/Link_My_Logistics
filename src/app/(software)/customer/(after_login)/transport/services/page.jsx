...
import { transport_services } from "@/constants/services";

export default function TransportServicesPage() {
	const router = useRouter();
	const { setTitle } = useSidebar();
	const [searchQuery, setSearchQuery] = useState('');
	const [filteredServices, setFilteredServices] = useState(transport_services);
	...
	const handleSearch = (e) => {
		...
		if (value === '') {
			setFilteredServices(transport_services);
		} else {
			setFilteredServices(transport_services.filter((service) => service.title.toLowerCase().includes(value.toLowerCase())));
		}
