...
import { services_3pl } from "@/constants/services";

export default function Services3PLPage() {
	const router = useRouter();
	const { setTitle } = useSidebar();
	const [searchQuery, setSearchQuery] = useState('');
	const [filteredServices, setFilteredServices] = useState(services_3pl);
	...
	const handleSearch = (e) => {
		...
		if (value === '') {
			setFilteredServices(services_3pl);
		} else {
			setFilteredServices(services_3pl.filter((service) => service.title.toLowerCase().includes(value.toLowerCase())));
		}
