...
export default function Form({ serviceName = '' }) {
  ...
  const { createItem, mutation } = useCollection('warehouse_service_details');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const type = warehouseServices.find((service) => service?.expand?.service?.title === 'Warehouse' && service?.title === serviceName)
      if (type?.id) {
