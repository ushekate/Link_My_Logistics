...
export default function OrderMovementViewDetailsPage() {
  ...
  const { data: orders, isLoading } = useCollection('cfs_order_movement', {
  ...
  return (
    <div className="max-w-container mx-auto px-6 py-10 space-y-8 bg-[color:var(--accent)] rounded-lg shadow-lg">
      ...
      <section>
        ...
        {order.expand?.order?.expand?.cfs ? (
          <div className="bg-[color:var(--background-2)] p-5 rounded-md shadow-sm border border-[color:var(--primary)] space-y-2">
            <DetailCard label="Title" value={order.expand.order.expand.cfs.title} />
            <DetailCard label="Location" value={order.expand.order.expand.cfs.location} />
            <DetailCard label="Contact" value={order.expand.order.expand.cfs.contact} />
          </div>
        ) : (
          <p className="text-[color:var(--secondary)]">CFS information not available.</p>
        )}
      </section>
