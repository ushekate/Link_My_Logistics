...
export default function AccountSetup() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen">
      <div className="hidden bg-gradient-to-r from-primary/100 to-80% w-1/2 lg:flex flex-col justify-center items-center">
      ...
      <div className="bg-white w-full lg:w-1/2 h-full overflow-y-auto">
        <div className="px-6 sm:px-10 lg:px-20 xl:px-40 mt-5 w-full max-w-screen-md mx-auto">
          ...
          <div className="bg-[#FDFDFD] flex flex-col justify-center items-center p-7 shadow-md mt-10">
            ...
            <p className="text-primary mt-2 mb-5 text-center">Set up your credentials and upload required documents</p>

            <div className="flex flex-col gap-2 w-full">

              {["License", "GST Certificate", "ID Proof"].map((label) => (
                <div key={label} className="mt-4">
                  <label className="text-primary text-sm font-medium w-full">
            ...
            <div className="flex gap-10 mt-5">
              ...
              <Link href="/client/Registration/Success">
                <button onClick={handleSubmit}
                  className={`h-9 w-40 flex justify-center items-center rounded-sm gap-1 ${agreed ? 'bg-primary text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>Submit<ArrowRight size={18} /></button>
              </Link>
            </div>
          </div>
        </div>
