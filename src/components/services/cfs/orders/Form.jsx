...
export default function Form() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const order = await createItem(data);
      if (order?.id) {
  ...
  const filteredServiceProviders = serviceProviders?.filter((p) => (p?.verified === true && p?.expand?.service?.some((s) => s.title === 'CFS')));
        ...
        <div className='flex flex-col gap-2'>
          <Label title={'CFS Facility'} />
          {
            filteredServiceProviders?.length > 0 && (
              <Select
                value={formData.cfs}
                onValueChange={(value) => setFormData({ ...formData, cfs: value })}
                placeholder='Select CFS Facility'
              >
