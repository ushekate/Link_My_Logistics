...
export default function Form({ service, serviceName }) {
  const { data: services } = useCollection('services');
  const { data: subServices } = useCollection('sub_services', {
    expand: 'service'
  });
  const { createItem, mutation } = useCollection('3pl_service_requests');
  const { user } = useAuth();
    ...
    try {
      ...
      if (request?.id) {
        const fetchedData = await fetchData({
          collectionName: '3pl_service_requests',
          options: {
        ...
        if (fetchedData?.length === 1) {
          ...
          createGeneralAuditLog({
            action: AUDIT_ACTIONS.CREATE,
            module: `3PL ${requestInfo?.expand?.service?.title}`,
  ...
  useEffect(() => {
    ...
    if (serviceName && subServices?.length > 0) {
      const foundedService = subServices?.find((s) => s?.title === serviceName);
      if (foundedService) {
        setFormData({ ...formData, serviceType: foundedService?.id });
        setLoading(false);
      }
    }
  }, [service, services]);
