...
export default function TrackTrace3PL({ service = '' }) {
	const [order, setOrder] = useState({});
	const [movements, setMovements] = useState([]);

	const { data: orders } = useCollection('3pl_orders', {
		expand: 'provider,containers'
	});
	const { data: movements_ } = useCollection('3pl_order_movement', {
		expand: 'order,service'
	});
	...
	const handleTrack = () => {
		...
		if (filtered?.id) {
			console.log(movements_)
			setMovements(movements_.filter((movement) => movement?.order === filtered?.id && movement?.expand?.service?.title === service))
			setOrder(filtered);
		} else {
			setOrder({})
		}
