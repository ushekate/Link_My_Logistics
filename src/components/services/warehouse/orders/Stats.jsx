...
export default function Stats({ original, orders, setOrders }) {
	...
	return (
		<div className="w-full grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-8 cursor-pointer">
			<div
				onClick={() => setOrders(original.filter(order => order?.status === 'Accepted'))}
			...
			<div
				onClick={() => setOrders(original.filter(order => order?.status === 'Pending'))}
				className="border rounded-lg p-6 grid gap-2 shadow-md shadow-foreground/40 bg-[var(--accent)]"
			>
				<div className="flex items-center justify-between">
			...
			<div
				onClick={() => setOrders(original.filter(order => order?.status === 'In Progress'))}
				className="border rounded-lg p-6 grid gap-2 shadow-md shadow-foreground/40 bg-[var(--accent)]"
			>
				<div className="flex items-center justify-between">
			...
			<div
				onClick={() => setOrders(original.filter(order => order?.status === 'Rejected'))}
				className="border rounded-lg p-6 grid gap-2 shadow-md shadow-foreground/40 bg-[var(--accent)]"
			>
				<div className="flex items-center justify-between">
