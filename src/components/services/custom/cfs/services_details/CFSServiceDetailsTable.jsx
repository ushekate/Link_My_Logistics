...
export default function CFSServiceDetailsTable({ serviceName = '' }) {
	const { data, deleteItem } = useCollection('custom_cfs_service_details', {
	...
	const redirectLink = (id) => {
		switch (user?.role) {
			case 'Merchant':
				return `/client/cfs/services/view/${id}`
			case 'Customer':
				return `/customer/cfs/services/view/${id}`
			case 'Root':
				return `/gol/cfs/services/view/${id}`
			default:
				return ''
		}
	}
		...
		if (data?.length > 0 && user?.id) {
			...
			switch (user?.role) {
				case 'Customer':
					...
					break;
				case 'Merchant':
					filtered_data = data.filter((item) => item?.expand?.type?.title === serviceName && item?.expand?.order?.expand?.cfs?.author === user?.id)
					break;
				case 'Root':
