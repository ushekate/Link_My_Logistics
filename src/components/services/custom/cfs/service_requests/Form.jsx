...
export default function Form({ serviceName = '' }) {
  ...
  const { createItem, mutation } = useCollection('custom_cfs_service_requests');
  const { user } = useAuth();
    ...
    try {
      ...
      if (request?.id) {
        const fetchedData = await fetchData({
          collectionName: 'custom_cfs_service_requests',
          options: {
                  ...
                  <Select value={formData.serviceType} onValueChange={handleServiceChange} placeholder='Select a Service'>
                    {
                      subServices
                        .filter((service) =>
                        (service?.expand?.service?.title === 'CFS' && (service?.title !== 'Job Order Update')
                        ))
                        .map((service, index) => (
