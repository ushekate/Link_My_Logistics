...
export default function CustomTransportOrderTable() {
  const { data, deleteItem, updateItem, mutation } = useCollection('custom_transport_orders', {
    expand: 'provider'
  });
  const { user } = useAuth();
  ...
  const redirectLink = (id) => {
    switch (user?.role) {
      case 'Merchant':
        return `/client/custom/services/transport/orders/view/${id}`
      case 'Customer':
        return `/customer/custom/services/transport/orders/view/${id}`
      case 'Root':
        return `/gol/custom/services/transport/orders/view/${id}`
      default:
        return ''
    }
  }
