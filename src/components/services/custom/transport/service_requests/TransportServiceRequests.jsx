...
// import { CircleCheckBig, Download, Eye, MessageCircleQuestion, Trash, } from 'lucide-react';
...
export default function CustomTransportServiceRequests({ serviceName = '' }) {
	const { data, updateItem, mutation, deleteItem } = useCollection('custom_transport_service_requests', {
	...
	const redirectLink = (id) => {
		switch (user?.role) {
			case 'Merchant':
				return `/client/transport/requests/view/${id}`
			case 'Customer':
				return `/customer/transport/requests/view/${id}`
			case 'Root':
				return `/gol/transport/requests/view/${id}`
			default:
				return ''
		}
	}
