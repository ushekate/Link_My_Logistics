...
export default function RejectForm({ info }) {
	const { updateItem, mutation } = useCollection('custom_transport_service_requests');
	...
	const handleSubmit = async (e) => {
		...
		try {
			switch (user?.role) {
				case 'Root':
					const updatedRequest = await updateItem(formData.id, {
					...
					await createGeneralAuditLog({
						...
						subModule: `${info?.expand?.serviceType?.title} Request`,
