...
import { sendServiceRequestEmail } from "@/app/action";
...
export default function Form({ serviceName = '' }) {
  ...
  const { createItem, mutation } = useCollection('custom_transport_service_requests');
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const request = await createItem(data);
      if (request?.id) {
        const fetchedData = await fetchData({
          collectionName: 'custom_transport_service_requests',
          options: {
            filter: `id='${request?.id}'`,
            expand: 'user,order,order.provider,serviceType'
          }
        });
        if (fetchedData?.length === 1) {
          const requestInfo = fetchedData[0];
          await createGeneralAuditLog({
            action: AUDIT_ACTIONS.CREATE,
            module: AUDIT_MODULES.CUSTOM_TRANSPORT,
            subModule: `${serviceName} Request`,
            details: requestInfo
          });
          await sendServiceRequestEmail({
            email: requestInfo?.expand?.user?.email,
            request: requestInfo,
            service: 'Custom Transport'
          });
        }
  ...
  useEffect(() => {
    if (serviceName) {
      const service_detail = subServices?.find((service) => service?.title === serviceName);
      if (service_detail?.id) {
        setFormData({ ...formData, serviceType: service_detail.id });
      }
                  ...
                  <Select value={formData.serviceType} onValueChange={handleServiceChange} placeholder='Select a Service'>
                    {
                      subServices
                        .filter((service) =>
                          service?.expand?.service?.title === 'Transport' && (
