...
export default function Form() {
  ...
  const { createItem, mutation } = useCollection('custom_transport_job_order')
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append vehicle IDs as JSON
      data.append('vehicles', JSON.stringify(selectedVehicles));
      // Append each file
      ...
      const newJobOrder = await createItem(data);
    ...
    >
      <div>
        <form className="grid grid-cols-1 md:grid-cols-2 gap-4 min-w-[60dvw]">
            ...
            <Label title="Service Type" />
            {
              warehouseServices?.length > 0 && (
                <Select value={formData.serviceType} onValueChange={handleSelectChange} placeholder='Select a Service'>
                  {
                    warehouseServices
                      .filter((service) => service?.expand?.service?.title === 'Transport')
                      .map((service, index) => (
          ...
          <div className='flex flex-col gap-2'>
            <Label title={'vehicles'} />
            <MultiSelectDatalist
              label="Select vehicles"
              options={vehicles}
              value={selectedVehicles}
              onValueChange={setSelectedVehicles}
              getOptionLabel={(vehicle) => `${vehicle.vehicleNo} - ${vehicle.name}`}
