...
export default function CustomPricingsRequests() {
	const { data, deleteItem, updateItem, mutation } = useCollection('custom_pricing_request', {
		expand: 'user,package'
	});
	...
	const columns = [
		...
		{
			id: 'package',
			accessorKey: 'expand.package.title',
			header: 'Package',
			filterable: true,
			cell: ({ row }) => <div>{row.original?.expand?.package?.title}</div>,
		},
		{
			id: 'startDate',
