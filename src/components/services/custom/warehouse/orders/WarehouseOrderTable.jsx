...
export default function CustomWarehouseOrderTable() {
	const { data, deleteItem, updateItem, mutation } = useCollection('custom_warehouse_orders', {
	...
	const redirectLink = (id) => {
		switch (user?.role) {
			case 'Merchant':
				return `/client/custom/services/warehouse/orders/view/${id}`
			case 'Customer':
				return `/customer/custom/services/warehouse/orders/view/${id}`
			case 'Root':
				return `/gol/custom/services/warehouse/orders/view/${id}`
			default:
				return ''
		}
	}
