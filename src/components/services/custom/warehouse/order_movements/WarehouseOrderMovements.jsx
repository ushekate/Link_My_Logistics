...
export default function CustomWarehouseOrderMovements() {
	const { data, deleteItem } = useCollection('custom_warehouse_order_movement', {
	...
	const redirectLink = (id) => {
		switch (user?.role) {
			case 'Merchant':
				return `/client/warehouse/order-movement/view/${id}`
			case 'Customer':
				return `/customer/warehouse/order-movement/view/${id}`
			case 'Root':
				return `/gol/warehouse/order-movement/view/${id}`
			default:
				return ''
		}
	}
