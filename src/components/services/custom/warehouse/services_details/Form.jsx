...
export default function Form({ serviceName = '' }) {
  ...
  const { createItem, mutation } = useCollection('custom_warehouse_service_details');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const type = subServices.find((service) => service?.expand?.service?.title === 'Warehouse' && service?.title === serviceName)
      if (type?.id) {
