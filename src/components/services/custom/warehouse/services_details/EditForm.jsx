...
export default function EditForm({ info }) {
  const { updateItem, mutation } = useCollection('custom_warehouse_service_details');
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedServiceDetail = await updateItem(formData.id, {
      ...
      await createGeneralAuditLog({
        ...
        subModule: info?.expand?.serviceType?.title,
        details: updatedServiceDetail
      });
