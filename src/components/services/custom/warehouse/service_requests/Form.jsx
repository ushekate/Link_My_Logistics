...
export default function Form({ serviceName = '' }) {
  ...
  const { createItem, mutation } = useCollection('custom_warehouse_service_requests');
  const { user } = useAuth();
    ...
    try {
      ...
      if (request?.id) {
        const fetchedData = await fetchData({
          collectionName: 'custom_warehouse_service_requests',
          options: {
        ...
        if (fetchedData?.length === 1) {
          ...
          await createGeneralAuditLog({
            ...
            module: AUDIT_MODULES.CUSTOM_WAREHOUSE,
