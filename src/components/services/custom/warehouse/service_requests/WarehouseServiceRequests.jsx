...
// import { CircleCheckBig, CircleX, Download, Eye, MessageCircleQuestion, Trash, } from 'lucide-react';
...
export default function CustomWarehouseServiceRequests({ serviceName = '' }) {
  const { data, updateItem, mutation, deleteItem } = useCollection('custom_warehouse_service_requests', {
  ...
  const redirectLink = (id) => {
    switch (user?.role) {
      case 'Merchant':
        return `/client/warehouse/requests/view/${id}`
      case 'Customer':
        return `/customer/warehouse/requests/view/${id}`
      case 'Root':
        return `/gol/warehouse/requests/view/${id}`
      default:
        return ''
    }
  }
